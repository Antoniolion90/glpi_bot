...
async def _menu_glpi_ticket(message: Message):
    ...
    await message.answer('Напишите название заявки:')

@dp.message_handler(state=AddTicket.AT1)
async def _default_name_ticket(message: Message, state: FSMContext):
    ticket_name = message.text

    async with state.proxy() as data:
        data['answer1'] = ticket_name

    await message.answer('Опишите свою проблему:')
    await AddTicket.next()

@dp.message_handler(state=AddTicket.AT2)
async def _default_content_ticket(message: Message, state: FSMContext):
    ticket_content = message.text

    await state.update_data(answer2=ticket_content)

    await message.answer('Выберите тип заявки:', reply_markup=get_type_ticket_markup())
    await AddTicket.next()

@dp.callback_query_handler(Regexp('type_'), state=AddTicket.AT3)
async def _change_glpi_ticket_type(callback_query: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(answer3=ticket_type)

    await callback_query.message.answer('Выберите срочность:', reply_markup=get_urgency_ticket_markup())
    await AddTicket.next()

@dp.callback_query_handler(Regexp('urgency_'), state=AddTicket.AT4)
async def _change_glpi_urgency_ticket(callback_query: CallbackQuery, state: FSMContext, user: User):
    await callback_query.message.edit_reply_markup(reply_markup=None)

    ticket_urgency = callback_query.data[8:]

    data = await state.get_data()
    ticket_name = data.get('answer1')
    ticket_content = data.get('answer2')
    ticket_type = data.get('answer3')
